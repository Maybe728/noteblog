package me.java.noteblog.controller.common;
...
public abstract class BaseController {
     ...
     * @return
     */
    public ResultBean handle(boolean res, String ok, String err) {
        return res ? ResultBean.ok(ok) : ResultBean.error(err);
    }
     ...
     * @return
     */
    protected ResultBean ajaxJsr303(List<FieldError> fieldErrors) {
        StringBuilder message = new StringBuilder();
        for (FieldError error : fieldErrors) {
            message.append(error.getField()).append(":").append(error.getDefaultMessage()).append("<br/>");
        }
        return ResultBean.error(message.toString());
    }
