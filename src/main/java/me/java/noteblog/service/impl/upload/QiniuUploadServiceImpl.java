...
public class QiniuUploadServiceImpl extends ServiceImpl<UploadMapper, Upload> implements UploadService<Object> {
    ...
    public <S> Object upload(Long sessionUserId, MultipartFile fileObj, String reqType, Consumer<S> extra, S s) {
        ...
        if (LAYUI_UPLOADER.equalsIgnoreCase(reqType)) {
            ...
            return ResultBean.error("上传至七牛服务器失败，错误信息：[{}]" + message);
        }
    }

    @Override
    public <S> Upload uploadIt(Long sessionUserId, MultipartFile fileObj, Consumer<S> extra, S t) {
        log.info("上传[" + fileObj.getContentType() + "]类型文件");
        Response res = doUpload(fileObj, getUpToken());
        try {
            if (res != null && res.isOK()) {
